...
'''
    ...
    def test_generate_tool_tests_integration(self, tool_mapper):
        """Integration test that verifies the complete test generation flow"""
        # This test uses the actual template structure and verifies
        # that the generated test code would be syntactically valid Python
        
        with tempfile.TemporaryDirectory() as temp_dir:
            template_dir = Path(temp_dir)
            
            # Minimal but complete test template
            template_content = '''
import pytest
...
class TestGeneratedMCPTools:
    ...
    {% for tool in tool_definitions %}
    @pytest.mark.asyncio
    async def test_{{ tool.name }}_success(self):
        """Test {{ tool.name }} tool with valid parameters"""
        # Test implementation for {{ tool.description }}
        assert True
    
    @pytest.mark.asyncio
    async def test_{{ tool.name }}_error(self):
        """Test {{ tool.name }} tool error handling"""
        # Error test for {{ tool.description }}
        assert True
    {% endfor %}
'''
            
            template_file = template_dir / "tool_tests.py.j2"
            template_file.write_text(template_content)
            
            with patch('os.path.join') as mock_join:
                mock_join.return_value = str(template_dir)
                
                result = tool_mapper.generate_tool_tests()
                
                # Verify the generated test code is syntactically valid
                try:
                    compile(result, '<generated_tests>', 'exec')
                    syntax_valid = True
                except SyntaxError:
                    syntax_valid = False
                
                assert syntax_valid, "Generated test code should be syntactically valid Python"
                
                # Verify key test components are present
                assert "class TestGeneratedMCPTools:" in result
                assert "@pytest.fixture" in result
                assert "def mock_server(self):" in result
                assert "def mock_api_client(self):" in result
                
                # Verify individual tool test methods
                assert "async def test_list_pets_success(self):" in result
                assert "async def test_get_pet_by_id_success(self):" in result
                assert "async def test_create_pet_success(self):" in result
                
                # Verify error test methods
                assert "async def test_list_pets_error(self):" in result
                assert "async def test_get_pet_by_id_error(self):" in result
                assert "async def test_create_pet_error(self):" in result
                
                # Verify test descriptions contain tool descriptions
                assert "List all pets" in result
                assert "Get pet by ID" in result
                assert "Create a pet" in result 