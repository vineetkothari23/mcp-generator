"""
lynnx-blog-mcp-server - Configuration Tests
...
class TestLynnx_blog_mcp_serverConfig:
    """Test cases for Lynnx_blog_mcp_serverConfig"""
    
    def test_default_configuration(self):
        """Test configuration with default values"""
        config = Lynnx_blog_mcp_serverConfig()
        
        assert config.server_name == "lynnx-blog-mcp-server"
    ...
    def test_custom_configuration(self):
        ...
        config = Lynnx_blog_mcp_serverConfig(**custom_config)
    ...
    def test_api_url_validation(self):
        ...
        for url in valid_urls:
            config = Lynnx_blog_mcp_serverConfig(api_base_url=url)
            assert config.api_base_url == url.rstrip('/')
        
        # Invalid URLs
        with pytest.raises(ValidationError):
            Lynnx_blog_mcp_serverConfig(api_base_url="invalid-url")
        
        with pytest.raises(ValidationError):
            Lynnx_blog_mcp_serverConfig(api_base_url="ftp://example.com")
    
    def test_timeout_validation(self):
        """Test timeout validation"""
        # Valid timeout
        config = Lynnx_blog_mcp_serverConfig(timeout=30)
        assert config.timeout == 30
        
        # Invalid timeout (negative)
        with pytest.raises(ValidationError):
            Lynnx_blog_mcp_serverConfig(timeout=-1)
        
        # Invalid timeout (zero)
        with pytest.raises(ValidationError):
            Lynnx_blog_mcp_serverConfig(timeout=0) 